    ...
    <div class="row g-4 justify-content-center">
        ...
        <div class="col-md-4">
            <div class="feature-card">
                ...
                <a href="/manifiesto-ia" class="btn-outline-alx">{{ \Alxarafe\Lib\Trans::_('philosophy') }} <i class="fas fa-arrow-right ms-2"></i></a>
            </div>
        </div>
                        ...
                        <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden card-hover">
                            <div class="card-body p-4">
                                <div class="d-flex justify-content-between align-items-start mb-3">
                                    <span class="badge bg-primary-soft text-primary small">{{ $post->published_at ? $post->published_at->format('d M, Y') : 'Borrador' }}</span>
                                    @if($post->tags->count() > 0)
                                        <div class="text-end">
                                            @foreach($post->tags->take(2) as $tag)
                                                <span class="badge rounded-pill bg-light text-secondary border small" style="font-size: 0.65rem;">{{ $tag->name }}</span>
                                            @endforeach
                                        </div>
                                    @endif
                                </div>
