...
Cada paquete nuevo contiene `.chascarrillo-managed-files.json`, con versión, rutas relativas y SHA-256. Tras una actualización correcta se conserva como manifiesto de la versión instalada.

El algoritmo es:

1. validar estructura, Composer, plantillas y manifiesto del paquete;
...
Un fallo de lectura, copia, publicación, limpieza o validación lanza un error y evita el mensaje de éxito. Las migraciones también deben devolver éxito. El log y el mensaje de mantenimiento deben conservar el primer error accionable.
